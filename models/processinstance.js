...
ProcessInstanceSchema.virtual('url').get(function() {
  return '/dashboard/processinstance/' + this._id;
});

ProcessInstanceSchema.virtual('scheduled_end_formatted').get(function() {
  return moment(this.scheduled_end).format('MMMM Do, YYYY');
});

ProcessInstanceSchema.virtual('scheduled_end_yyyy_mm_dd').get(function() {
