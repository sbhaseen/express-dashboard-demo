...
exports.machine_list = function(req, res, next) {
    ...
    Machine.find()
      ...
      .exec(function (err, list_machines) {
        ...
        res.render('machine_list', { title: 'Machine List', machine_list: list_machines });
      });
...
exports.machine_detail = function(req, res, next) {
    ...
    async.parallel({
        ...
        machines_processes: function(callback) {
          Process.find({ 'machine': req.params.id },'name summary')
          .exec(callback)
        },
        ...
        // Successful, so render.
        res.render('machine_detail', { title: 'Machine Detail',
...
exports.machine_create_post = [
    ...
    body('date_of_commissioning', 'Invalid date of commissioning').optional({ checkFalsy: true }).isISO8601(),
    body('date_of_retirement', 'Invalid date of retirement').optional({ checkFalsy: true }).isISO8601(),

    // Sanitize fields.
    sanitizeBody('name').escape(),
    sanitizeBody('date_of_commissioning').toDate(),
    sanitizeBody('date_of_retirement').toDate(),
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            res.render('machine_form', { title: 'Create Machine', machine: req.body, errors: errors.array() });
            return;
        }
        else {
...
exports.machine_delete_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        // Successful, so render.
        res.render('machine_delete', { title: 'Delete Machine', machine: results.machine, machine_processess: results.machines_processes } );
    });
...
exports.machine_delete_post = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (results.machines_processes.length > 0) {
            ...
            res.render('machine_delete', { title: 'Delete Machine', machine: results.machine, machine_processes: results.machines_processes } );
            return;
        }
        else {
            // Machine has no processess. Delete object and redirect to the list of machines.
            Machine.findByIdAndRemove(req.body.machineid, function deleteMachine(err) {
                if (err) { return next(err); }
...
exports.machine_update_post = [
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            res.render('machine_form', { title: 'Update Machine', machine: machine, errors: errors.array() });
            return;
        }
        else {
            // Data from form is valid. Update the record.
            Machine.findByIdAndUpdate(req.params.id, machine, {}, function (err, themachine) {
                if (err) { return next(err); }
